...
def test_ai_implement_with_cache_enabled(mock_openai):
    # Setup mock to return valid python code
    mock_response = MagicMock()
    mock_response.choices[0].message.content = "return a + b"
    mock_openai.chat.completions.create.return_value = mock_response
    ...
    with patch("cursed_vibing_on_the_fly.core._CACHE_ENABLED", True):

        @ai_implement
        def add_cached(a: int, b: int) -> int:
            pass

        assert add_cached(1, 2) == 3
        mock_openai.chat.completions.create.assert_called_once()

        # Verify caching (calling again should not trigger API call)
        assert add_cached(3, 4) == 7
        mock_openai.chat.completions.create.assert_called_once()
